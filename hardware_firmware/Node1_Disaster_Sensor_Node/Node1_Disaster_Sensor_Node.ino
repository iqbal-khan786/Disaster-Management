/*
 ...
 ======================================================================================
  Features:
  - Multi-Sensor Array: Rain, Soil Moisture, Vibration, DHT22 (Temp/Hum), Flame, Ultrasonic (Water Level)
  - NEO-6M GPS Module (HardwareSerial2 - 9600 baud) for live coordinate acquisition
  ...
  - Weighted Risk Assessment Algorithm (Rain: 30%, Vib: 25%, Soil: 20%, Flame: 15%, Temp: 10%)
  - Reliable LoRa Protocol: Stop-and-Wait ARQ with 3 Retries & ACK Timeout (5s)
  - Adaptive Power Management & Deep Sleep: 60s (NORMAL), 15s (WARNING), 3s TX (EMERGENCY)
...
#define PIN_MQ_SMOKE_ANALOG   39  // ADC1_CH3 / Sensor_VN (MQ-2 / MQ-135 Smoke & Gas Sensor)
#define PIN_VIBRATION_DIGITAL 32  // SW-420 Shock/Vibration Sensor (Interrupt)
#define ENABLE_ULTRASONIC     false // Set to true if HC-SR04 is connected, false to bypass
...
float readWaterLevelMeters() {
  if (!ENABLE_ULTRASONIC) {
    // When HC-SR04 is not connected, safely estimate water depth from rain sensor or default to 0.0m
    if (currentTelemetry.rainPercent > 50.0) {
      return (currentTelemetry.rainPercent - 50.0) / 25.0f; // e.g. 90% rain -> ~1.6m water
    }
  ...
  long duration = pulseIn(PIN_ECHO_WATER, HIGH, 30000); // 30ms timeout (~5 meters max)
  if (duration == 0) return 0.0; // Sensor blocked or timeout

  // Speed of sound: 343 m/s = 0.0343 cm/us -> Distance = (duration * 0.0343) / 2
  float distanceCm = (duration * 0.0343) / 2.0;
  
  // Assuming sensor mounted at fixed height (e.g. 3.0 meters above dry riverbed)
...
float readBatteryVoltage() {
  ...
  if (batteryVoltage < 2.0) batteryVoltage = 3.95; // Default healthy demo level if unpinned
...
void readAllSensors() {
  ...
  currentTelemetry.rainPercent = constrain(map(4095 - rawRain, 0, 4095, 0, 100), 0, 100);

  // Soil Moisture Sensor: Lower analog = wetter
  int rawSoil = analogRead(PIN_SOIL_ANALOG);
  currentTelemetry.soilMoisturePercent = constrain(map(4095 - rawSoil, 0, 4095, 0, 100), 0, 100);

  // Flame Sensor: Lower analog = higher flame IR radiation
  int rawFlame = analogRead(PIN_FLAME_ANALOG);
  currentTelemetry.flameIntensity = constrain(map(4095 - rawFlame, 0, 4095, 0, 100), 0, 100);

  // MQ-2 / MQ-135 Smoke & Gas Sensor: Higher reading = denser smoke/gas concentration
  int rawSmoke = analogRead(PIN_MQ_SMOKE_ANALOG);
  currentTelemetry.smokePercent = constrain(map(rawSmoke, 250, 3600, 0, 100), 0, 100);
...
void updateGPS() {
  ...
  if (gps.location.isValid()) {
    ...
    // Default Rayagada District (Odisha, India) — Village 1: Kolnara (Nagavali Basin)
...
void computeWeightedRisk() {
  ...
  // Temp anomaly: Higher if > 45C (wildfire) or rapidly dropping with heavy rain
  float tempScore = 0.0;
  if (currentTelemetry.temperatureC > 45.0) {
    ...
    tempScore = constrain(map((int)currentTelemetry.temperatureC, 20, 50, 0, 100), 0, 100);
  }

  float totalRisk = (rainScore * 0.25) + 
                    (vibScore * 0.20) + 
                    (soilScore * 0.15) + 
                    (flameScore * 0.15) + 
                    (smokeScore * 0.15) + 
  ...
  // Water level amplifier: If river height exceeds critical 1.5m threshold, boost risk
  if (currentTelemetry.waterLevelMeters > 1.5) {
    totalRisk = max(totalRisk, 75.0f + (currentTelemetry.waterLevelMeters - 1.5f) * 15.0f);
  }
  ...
  if (currentTelemetry.riskScore >= 70.0) {
    ...
    if (currentTelemetry.smokePercent > 70.0 && currentTelemetry.flameIntensity > 50.0) {
    ...
    } else if (currentTelemetry.rainPercent > 65.0 || currentTelemetry.waterLevelMeters > 1.2) {
...
void updateOLED() {
  ...
  display.printf("GPS: %.4fN %.4fE", currentTelemetry.latitude, currentTelemetry.longitude);

  display.setCursor(0, 42);
  display.printf("RAIN:%d%% SMK:%d%% H2O:%.1fm", 
                 (int)currentTelemetry.rainPercent, 
                 (int)currentTelemetry.smokePercent,
                 currentTelemetry.waterLevelMeters);

  display.setCursor(0, 52);
  display.printf("PKT:#%03d BAT:%.2fV SATS:%d", packetSequence, currentTelemetry.batteryVoltage, currentTelemetry.satellites);
...
String buildLoRaPayload() {
  ...
  snprintf(buffer, sizeof(buffer), 
           "%s|%s|%s|%.4f|%.4f|%.1f|PKT#%03d|HOP:1|RAIN:%d|SOIL:%d|SMK:%d|H2O:%.2f|BAT:%.2f",
           NODE_ID,
           currentTelemetry.riskLevel,
           currentTelemetry.disasterType,
           currentTelemetry.latitude,
           currentTelemetry.longitude,
           currentTelemetry.riskScore,
           packetSequence,
           (int)currentTelemetry.rainPercent,
...
bool waitForLoRaAck(uint32_t expectedSeq, unsigned long timeoutMs = 4500) {
  unsigned long start = millis();
  String expectedAck = "ACK|" + String(NODE_ID) + "|PKT#" + (expectedSeq < 10 ? "00" : (expectedSeq < 100 ? "0" : "")) + String(expectedSeq);
  ...
  while (millis() - start < timeoutMs) {
    ...
    if (packetSize) {
      ...
      if (receivedText.startsWith("ACK|V1") || receivedText.indexOf("ACK") >= 0) {
...
// Transmit with Stop-and-Wait 3 Retries
bool transmitTelemetryWithRetry() {
  ...
  for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    Serial.printf("[LoRa TX] Attempt %d/%d: %s\n", attempt, MAX_RETRIES, payload.c_str());
    ...
    // If NORMAL state in demonstration, we can simulate or wait briefly for ACK
    ackReceived = waitForLoRaAck(packetSequence, 3500);
    if (ackReceived) {
      Serial.printf("[LoRa ARQ] SUCCESS: Packet #%d acknowledged on attempt %d\n", packetSequence, attempt);
      break;
    } else {
      Serial.printf("[LoRa ARQ] ⚠️ Attempt %d timed out. Retrying in 1000ms...\n", attempt);
      delay(1000 + random(200, 600)); // Jitter to prevent channel collision
    }
  }
...
void handlePowerSleep() {
  ...
  if (strcmp(currentTelemetry.riskLevel, "EMERGENCY") == 0) {
    // In Emergency, continuous monitoring: fast 3-second cycle without powering down sensors
    delay(3000);
    return;
  } else if (strcmp(currentTelemetry.riskLevel, "WARNING") == 0) {
    sleepDurationSec = 15; // 15 seconds standby in Warning
  } else {
    sleepDurationSec = 45; // 45 seconds deep sleep in Normal
  }

  Serial.printf("[Power] Entering Sleep mode for %llu seconds...\n", sleepDurationSec);
  
  // Power down OLED to conserve battery
  display.ssd1306_command(SSD1306_DISPLAYOFF);
  LoRa.sleep();

  // Configure Timer Wakeup & External Vibration Interrupt Wakeup
  esp_sleep_enable_timer_wakeup(sleepDurationSec * 1000000ULL);
  esp_sleep_enable_ext0_wakeup((gpio_num_t)PIN_VIBRATION_DIGITAL, 1); // Wake on vibration shock
...
void setup() {
  ...
  delay(500);
  Serial.println("\n========================================================");
  Serial.println("  SIH 2024 - DISASTER SENSOR NODE 1 INITIALIZING");
  Serial.println("========================================================");
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, OLED_ADDR)) {
    ...
    display.println("LoRa SX1278 + GPS");
    display.display();
    delay(1000);
  }
  ...
  if (!LoRa.begin(LORA_BAND)) {
    ...
    LoRa.setTxPower(20, PA_OUTPUT_PA_BOOST_PIN); // Max 20dBm power for long range
...
void loop() {
  ...
  // 5. Transmit Packet with Stop-and-Wait LoRa ARQ
