 ...
 */
export function generateDisasterPDFReport({
  logs = [],
  currentNode = {},
  summaryStats = {},
}) {
  ...
  doc.setFontSize(14);
  doc.text(
    "GOVERNMENT OF ODISHA — DISASTER MANAGEMENT AUTHORITY (OSDMA)",
    14,
    11,
  );
  ...
  doc.text(
    "DISTRICT EMERGENCY OPERATION CENTER (DEOC) • RAYAGADA HEADQUARTERS",
    14,
    17,
  );
  doc.text(
    "High-Frequency IoT Telemetry & Multi-Hazard Sensor Early Warning Record",
    14,
    23,
  );
  ...
  doc.setFont("helvetica", "normal");
  doc.text(
    `${currentNode.accuracy || currentNode.dataAccuracy || 92.4}% (85% - 95% Band)`,
    col2X + 28,
    currentY + 21,
  );
  ...
  doc.text(
    "1. EXECUTIVE PHYSICAL SENSOR SUMMARY (CURRENT TELEMETRY)",
    14,
    currentY,
  );
  ...
  if (finalY < pageHeight - 35) {
    ...
    doc.text(
      "OFFICIAL VERIFICATION & DISASTER RESPONSE DIRECTIVE:",
      18,
      finalY + 5.5,
    );
    ...
    doc.setFont("helvetica", "normal");
    doc.text("STATUS: VERIFIED", pageWidth - 32, finalY + 16, {
      align: "center",
    });
  }
