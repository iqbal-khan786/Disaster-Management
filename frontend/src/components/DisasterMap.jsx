...
const RAYAGADA_SECTORS = [
  {
    name: "Village 1: Kashipur Valley (Live ESP32 Hardware Node)",
    lat: 19.195,
    lng: 83.395,
    zoom: 14,
  },
  {
    name: "District Headquarter: Rayagada DEOC Base Hub",
    lat: 19.167,
    lng: 83.416,
    zoom: 15,
  },
...
const NAGAVALI_RIVER_COORDS = [
  [19.26, 83.315],
  [19.248, 83.325],
  [19.24, 83.33], // Kashipur V1
  [19.231, 83.342],
  [19.218, 83.358],
  [19.205, 83.378],
  [19.195, 83.395], // Kolnara V2
  [19.182, 83.405],
  [19.173, 83.411],
  [19.167, 83.416], // Rayagada HQ
  [19.155, 83.422],
  [19.142, 83.43],
];
...
export function DisasterMap({
  ...
  const runMeshLifecycleDemo = () => {
    ...
    map.flyTo([19.24, 83.33], 14, { duration: 1.2 });

    // Step 2: LoRa Hop 1 (V1 -> V2) after 2.5 seconds
    setTimeout(() => {
      setSimulationStep(2);
      map.flyTo([19.195, 83.395], 13.5, { duration: 1.2 });
    }, 2800);

    // Step 3: LoRa Hop 2 (V2 -> HQ) after 5.5 seconds
    setTimeout(() => {
      setSimulationStep(3);
      map.flyTo([19.167, 83.416], 13.5, { duration: 1.2 });
    }, 5600);

    // Step 4: Rescue Dispatched (HQ -> V1) after 8.5 seconds
    setTimeout(() => {
      setSimulationStep(4);
      map.flyTo([19.2, 83.375], 12, { duration: 1.4 });
    }, 8500);
  ...
  const handleSectorChange = (index) => {
    ...
    if (mapInstanceRef.current) {
      mapInstanceRef.current.flyTo([spot.lat, spot.lng], spot.zoom, {
        duration: 1.2,
      });
    }
  };
  ...
  useEffect(() => {
    ...
    if (!mapInstanceRef.current) {
      ...
      map.on("mousemove", (e) => {
        ...
        const latRatio = (e.latlng.lat - 19.167) / (19.24 - 19.167);
        const estElev = Math.round(
          210 + latRatio * 280 + Math.sin(e.latlng.lng * 10) * 80,
        );
        setCursorCoords({
          lat: e.latlng.lat,
          lng: e.latlng.lng,
      ...
      riverLine.bindPopup(
        '<b style="color:#0284c7;">Nagavali River Basin</b><br/>Major flash flood drainage corridor in Rayagada',
      );
      ...
      stormPoly.bindPopup(
        '<b style="color:#818cf8;">Doppler Precipitation Radar</b><br/>Active Cloudburst Cell (>95mm/hr rain rate)',
      );
      ...
      L.marker([19.167, 83.416], { icon: hqIcon }).addTo(map).bindPopup(`
          <div style="font-family:sans-serif; padding:4px;">
      ...
      const circleV1 = L.circle([19.24, 83.33], {
        radius: 7500,
        color: "#ef4444",
        weight: 1,
        fillColor: "#ef4444",
      ...
      const circleV2 = L.circle([19.195, 83.395], {
        radius: 9500,
        color: "#10b981",
        weight: 1,
        fillColor: "#10b981",
      ...
      const circleHQ = L.circle([19.167, 83.416], {
        radius: 6000,
        color: "#0284c7",
        weight: 1,
        fillColor: "#0284c7",
    ...
    nodeList.forEach((node) => {
      ...
      const isWarn =
        ...
        (node.riskScore >= 40 && node.riskScore < 70);
      const color = isEmerg ? "#ef4444" : isWarn ? "#f59e0b" : "#10b981";
      const auraColor = isEmerg
        ? "rgba(239, 68, 68, 0.45)"
    ...
    if (node1) {
      const hopPath = [
        ...
        [19.167, 83.416], // Destination: District Headquarter (Rayagada DEOC)
      ];
    ...
    if (dispatches.length > 0 && nodes["V1"]) {
      const rescuePath = [
        [19.167, 83.416], // HQ Rayagada
        [19.195, 83.395], // Kolnara
        [19.225, 83.35], // River Waypoint
      ...
      if (!rescueMarkerRef.current) {
        rescueMarkerRef.current = L.marker([19.215, 83.365], {
          icon: rescueIcon,
        })
          .addTo(map)
          .bindPopup(
            '<b style="color:#ef4444;">NDRF Inflatable Rescue Boat</b><br/>En route to Village 1 via Nagavali water corridor',
          );
      }
    }
  }, [nodes]);
  ...
  useEffect(() => {
    if (floodPolygonRef.current) {
      if (showFloodZone)
        floodPolygonRef.current.setStyle({ opacity: 1, fillOpacity: 0.35 });
      else floodPolygonRef.current.setStyle({ opacity: 0, fillOpacity: 0 });
    }
    if (stormPolygonRef.current) {
      if (showStormRadar)
        stormPolygonRef.current.setStyle({ opacity: 1, fillOpacity: 0.2 });
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <div
            style={{
              background:
                "linear-gradient(135deg, rgba(56, 189, 248, 0.25), rgba(2, 132, 199, 0.25))",
              padding: "6px",
              borderRadius: "8px",
              color: "var(--accent)",
            }}
          ...
          <div>
            ...
            >
              {isHindi
                ? "रायगड़ा सामरिक राहत एवं मल्टी-हॉप मेश मैप"
                : "Rayagada Tactical Disaster GIS & LoRa Mesh Map"}
              <span
                style={{
                  fontSize: "10px",
            ...
            <p style={{ fontSize: "10px", color: "var(--text-muted)" }}>
              {isHindi
                ? "नागावली नदी बेसिन • लाइव जलप्रवाह एवं पहाड़ी रिले टोपोग्राफी"
                : "Nagavali River Basin • Live LoRa 433MHz Mesh Corridor (V1 ➔ V2 Relay ➔ DEOC Base)"}
            </p>
          </div>
        </div>
        ...
        >
          ...
          >
            <button
              ...
              style={{
                background:
                  activeLayer === "satellite" ? "var(--accent)" : "transparent",
                color:
                  activeLayer === "satellite" ? "#000" : "var(--text-muted)",
                border: "none",
                padding: "4px 8px",
                borderRadius: "4px",
                fontSize: "11px",
            ...
            <button
              ...
              style={{
                background:
                  activeLayer === "topo" ? "var(--accent)" : "transparent",
            ...
            <button
              ...
              style={{
                background:
                  activeLayer === "osm" ? "var(--accent)" : "transparent",
      ...
      >
        ...
        <button
          ...
          style={{
            background: showElevationProfile
              ? "rgba(16, 185, 129, 0.2)"
              : "rgba(255,255,255,0.05)",
        ...
        >
          ...
          >
            ...
            >
              <Mountain size={13} />{" "}
              {isHindi
                ? "पहाड़ी एलिवेशन क्रॉस-सेक्शन (LoRa रिले क्यों आवश्यक है)"
                : "Mountain Terrain Cross-Section & Line-of-Sight Profile"}
            </span>
            <span
              style={{
                fontSize: "10px",
          ...
          <div
            style={{
              ...
              height: "75px",
              background:
                "linear-gradient(180deg, rgba(15,23,42,0.6) 0%, rgba(4,7,17,0.9) 100%)",
              borderRadius: "6px",
          ...
          >
            ...
            >
              ...
              <path
                d="M 0,100 L 0,65 Q 60,40 100,55 Q 160,85 200,20 Q 260,75 320,60 Q 360,75 400,80 L 400,100 Z"
            ...
            <div style={{ zIndex: 2, textAlign: "center" }}>
              <div
                style={{ fontSize: "9px", fontWeight: 800, color: "#ef4444" }}
              >
                V1: Kashipur
              </div>
              <div
            ...
            <div style={{ zIndex: 2, textAlign: "center" }}>
              <div
                style={{ fontSize: "9px", fontWeight: 800, color: "#38bdf8" }}
              >
                V2: Kolnara Ridge
              </div>
              <div
            ...
            <div style={{ zIndex: 2, textAlign: "center" }}>
              <div
                style={{ fontSize: "9px", fontWeight: 800, color: "#10b981" }}
              >
                HQ: Rayagada DEOC
              </div>
              <div
          ...
          >
            <i>
              Why Relay? Direct signal from V1 to HQ is blocked by a 680m
              mountain ridge. Village 2 acts as a line-of-sight wireless
              repeater.
            </i>
          </div>
        </div>
      ...
      >
        ...
        >
          <span style={{ display: "flex", alignItems: "center", gap: "5px" }}>
            ...
            />
            {isHindi
              ? "V1 (काशीपुर घाटी - 460m)"
              : "V1: Kashipur (460m Flood Zone)"}
          </span>
          <span style={{ display: "flex", alignItems: "center", gap: "5px" }}>
            <span
              style={{
          ...
          <span style={{ display: "flex", alignItems: "center", gap: "5px" }}>
            ...
            />
            {isHindi
              ? "HQ (कलेक्ट्रेट बेस - 210m)"
              : "HQ: Rayagada DEOC (210m Base)"}
          </span>
          <span style={{ display: "flex", alignItems: "center", gap: "5px" }}>
            <span
              style={{ width: "12px", height: "3px", background: "#0284c7" }}
            />
