...
import { LifeBuoy, Send, Truck, Building2, Info } from "lucide-react";
import { DispatchModal } from "../DispatchModal";

export function RescueOperationsView({ nodes = {}, onOpenDispatch }) {
  const nodeList = Object.values(nodes);
  const criticalNode =
    nodeList.find((n) => n.riskLevel === "CRITICAL" || n.riskScore >= 75) ||
    nodeList[0] ||
    {};
      ...
      >
        <div>
          ...
          <p style={{ fontSize: "11px", color: "#94a3b8", margin: "3px 0 0" }}>
            Multi-Agency Dispatch Console • ODRAF, NDRF, Fire Service & District
            Medical Evacuation
          </p>
        </div>
      ...
      >
        ...
        <div style={{ fontSize: "11px", color: "#cbd5e1", lineHeight: "1.4" }}>
          <strong style={{ color: "#ef4444" }}>
            AI Dispatch Recommendation Note:
          </strong>{" "}
          Prioritization rankings and rescue recommendations are algorithmically
          generated suggestions based on incoming LoRa sensor telemetry. Final
          tactical routing, boat deployment, and evacuation decisions must be
          authenticated and directed by field commanders and DEOC officers.
        </div>
      </div>
          ...
          {nodeList.map((node) => (
            ...
            >
              ...
              >
                ...
                <div>
                  Rainfall: <strong>{node.rainMm || 0}mm/h</strong> | Soil:{" "}
                  <strong>{node.soilMoisture || 0}%</strong> | Smoke:{" "}
                  <strong>{node.smokeLevel || 0}PPM</strong>
                </div>
                <div>
                  Nearest Gateway:{" "}
                  <strong>Rayagada Base Hub (Hop {node.hopCount || 1})</strong>
                </div>
              </div>
              ...
              >
                <div>
                  ...
                  >
                    ...
                    • ETA:{" "}
                    <strong style={{ color: "#34d399" }}>{team.eta}</strong>
                  </div>
                </div>
