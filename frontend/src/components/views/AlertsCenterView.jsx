...
export function AlertsCenterView({
  ...
  const activeCriticalAlert = alerts.find(a => a.status === 'active' && (a.level === 'CRITICAL' || a.level === 'EMERGENCY'));
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div className="glass-panel" style={{ padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '14px' }}>
        <div>
          ...
          <p style={{ fontSize: '11px', color: '#94a3b8', margin: '3px 0 0' }}>
            Multi-Hazard Trigger Log, Village Siren Broadcasts & Operator Audit Logs
          </p>
        </div>
      ...
      {activeCriticalAlert && (
        <div className="alert-flash" style={{
          background: 'linear-gradient(90deg, rgba(239, 68, 68, 0.3) 0%, rgba(15, 23, 42, 0.95) 100%)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span style={{ fontSize: '14px', fontWeight: 900, color: '#fca5a5', textTransform: 'uppercase' }}>
                  {activeCriticalAlert.title}
                </span>
                <span style={{ background: '#ef4444', color: '#fff', fontSize: '9px', fontWeight: 800, padding: '2px 6px', borderRadius: '4px' }}>
                  IMMEDIATE ACTION
                </span>
              </div>
              <p style={{ fontSize: '11px', color: '#fecaca', margin: '4px 0 0' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button
              onClick={() => onAcknowledge && onAcknowledge(activeCriticalAlert.id)}
              className="action-btn"
              style={{
                background: '#38bdf8',
                color: '#000',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {filteredAlerts.length === 0 ? (
          <div className="glass-panel" style={{ padding: '30px', textAlign: 'center', color: '#64748b', fontSize: '13px' }}>
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                      ...
                      <span style={{
                        ...
                        padding: '2px 6px',
                        borderRadius: '4px',
                        background: alert.status === 'active' ? 'rgba(239, 68, 68, 0.2)' : alert.status === 'acknowledged' ? 'rgba(245, 158, 11, 0.2)' : 'rgba(16, 185, 129, 0.2)',
                        color: alert.status === 'active' ? '#ef4444' : alert.status === 'acknowledged' ? '#f59e0b' : '#10b981',
                    ...
                    <p style={{ fontSize: '11px', color: '#cbd5e1', margin: '4px 0 6px' }}>
                      {alert.desc}
                    </p>

                    <div style={{ display: 'flex', alignItems: 'center', gap: '14px', fontSize: '10px', color: '#64748b' }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <MapPin size={11} /> {alert.village || 'Rayagada Sector'}
                      </span>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <Radio size={11} /> {alert.triggerSensor || 'Ultrasonic + Rain'}
                      </span>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
