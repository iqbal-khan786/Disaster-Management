...
import { History, Download, FileText, FileDown, Search } from "lucide-react";
...
export function HistoricalDataView({ nodes = {} }) {
  ...
  const historicalLogs = React.useMemo(() => {
    ...
    if (timeRange === "6h")
      stepMs = 15 * 60 * 1000; // 15 mins interval
    else if (timeRange === "24h")
      stepMs = 60 * 60 * 1000; // 1 hour interval
    ...
    for (let i = 0; i < targetCount; i++) {
      ...
      if (timeRange === "7d") {
        ...
        formattedTime = recordDate.toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
          second: "2-digit",
        });
      ...
      const rowTemp = Number(
        Math.max(
          ...
          Math.min(
            38.0,
            baseTemp + solarFactor * 2.6 + Math.sin(i * 0.8) * 0.25,
          ),
        ).toFixed(1),
      );
      const rowHum = Number(
        Math.max(
          35.0,
          Math.min(95.0, 64.5 - solarFactor * 9.5 + Math.cos(i * 0.9) * 0.7),
        ).toFixed(1),
      );
      const rowSoil = Number(
        Math.max(
          25.0,
          Math.min(
            85.0,
            baseSoil + Math.sin(i * 0.35) * 1.1 + Math.cos(i * 0.7) * 0.3,
          ),
        ).toFixed(1),
      );
      const rowSmoke = Number(
        Math.max(
          14.0,
          Math.min(
            28.0,
            baseSmoke + Math.sin(i * 0.5) * 0.6 + (i % 3 === 0 ? 0.3 : -0.2),
          ),
        ).toFixed(1),
      );
      const rowRain = Number(
        Math.max(
          0.0,
          baseRain > 5
            ? Math.max(0, baseRain - i * 0.8)
            : i % 8 === 0
              ? 0.1
              : 0.0,
        ).toFixed(1),
      ...
      const rowRisk = Number(
        Math.max(
          5.0,
          Math.min(100.0, rainScore + soilScore + smokeScore + 1.8),
        ).toFixed(1),
      );
  ...
  const filteredLogs = historicalLogs.filter((log) => {
    if (selectedVillageFilter !== "all" && log.nodeId !== selectedVillageFilter)
      return false;
    if (
      searchQuery &&
  ...
  const avgSoil = (
    historicalLogs.reduce((acc, l) => acc + l.soilMoisture, 0) /
    (historicalLogs.length || 1)
  ).toFixed(1);
  const maxSmoke = Math.max(...historicalLogs.map((l) => l.smokeLevel), 0);
  const avgTemp = (
    historicalLogs.reduce((acc, l) => acc + l.temp, 0) /
    (historicalLogs.length || 1)
  ).toFixed(1);
  ...
  const exportCSV = () => {
    const headers =
      "Log ID,Timestamp,Node ID,Village,Rainfall (mm/h),Soil Moisture (%),Smoke (PPM),Flame Detected,Vibration Detected,Temp (C),Humidity (%),Risk Score,RSSI (dBm)\n";
    const rows = filteredLogs
      .map(
        (l) =>
          `${l.id},${l.time},${l.nodeId},"${l.village}",${l.rain},${l.soilMoisture},${l.smokeLevel},${l.flameDetected ? "YES" : "NO"},${l.vibration ? "YES" : "NO"},${l.temp},${l.humidity},${l.riskScore},${l.rssi}`,
      )
      .join("\n");
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: "11px", color: "#94a3b8", margin: "3px 0 0" }}>
            Stored 6-Sensor Environmental Records • Official OSDMA & DEOC Audit
            Generator
          </p>
        </div>
      ...
      >
        <div className="glass-card" style={{ padding: "14px" }}>
          ...
          >
            {maxRain}{" "}
            <span style={{ fontSize: "12px", color: "#64748b" }}>mm/h</span>
          </div>
        ...
        <div className="glass-card" style={{ padding: "14px" }}>
          ...
          >
            {maxSmoke}{" "}
            <span style={{ fontSize: "12px", color: "#64748b" }}>PPM</span>
          </div>
          <span style={{ fontSize: "10px", color: "#94a3b8" }}>
            MQ-2 Gas Sensor
          </span>
          ...
          {["1h", "6h", "24h", "7d"].map((range) => (
            <button
              ...
              style={{
                ...
                border: "none",
                background:
                  timeRange === range ? "#38bdf8" : "rgba(255, 255, 255, 0.04)",
                ...
                >
                  ...
                  <td
                    style={{
                      padding: "8px",
                      fontFamily: "JetBrains Mono",
                      color: log.flameDetected ? "#ef4444" : "#10b981",
                      fontWeight: 700,
                    }}
                  >
                  ...
                  <td
                    style={{
                      padding: "8px",
                      fontFamily: "JetBrains Mono",
                      color: log.vibration ? "#ef4444" : "#10b981",
                      fontWeight: 700,
                    }}
                  >
                  ...
                  <td style={{ padding: "8px", color: "#94a3b8" }}>
                    {log.temp}°C / {log.humidity}%
                  </td>
                  <td
                    style={{
                      padding: "8px",
                      fontFamily: "JetBrains Mono",
                      color: log.riskScore > 60 ? "#ef4444" : "#34d399",
                      fontWeight: 700,
                    }}
                  >
                    {log.riskScore}/100
