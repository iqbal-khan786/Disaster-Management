...
import { Layers, Thermometer, Droplets, Download } from "lucide-react";
...
export function GraphDetailsView({ history = [], nodes = {} }) {
  ...
  const chartDataPoints = useMemo(() => {
    ...
    if (timeRange === "1H") {
      ...
      for (let i = count - 1; i >= 0; i--) {
        const timeStr = new Date(now - i * stepMs).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        });
        const tVal = Number(
          (baseTemp + Math.sin((count - i) * 0.5) * 1.2).toFixed(1),
        );
        const hVal = Number(
          Math.min(
            100,
            Math.max(10, baseHum - Math.sin((count - i) * 0.5) * 3.2),
          ).toFixed(1),
        );
    ...
    if (timeRange === "6H") {
      ...
      for (let i = count - 1; i >= 0; i--) {
        ...
        const tVal = Number(
          (baseTemp + Math.sin((count - i) * 0.4) * 2.2).toFixed(1),
        );
        const hVal = Number(
          Math.min(
            100,
            Math.max(10, baseHum - Math.sin((count - i) * 0.4) * 6.5),
          ).toFixed(1),
        );
        points.push({ label: timeStr, temp: tVal, hum: hVal });
      }
    ...
    if (timeRange === "24H") {
      ...
      return hours.map((h, idx) => {
        const tVal = Number(
          (baseTemp + Math.cos((idx - 4) * 0.7) * 4.2).toFixed(1),
        );
        const hVal = Number(
          Math.min(
            95,
            Math.max(25, baseHum - Math.cos((idx - 4) * 0.7) * 12.0),
          ).toFixed(1),
        );
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        points.push({
          ...
          hum: Number(
            Math.min(
              95,
              Math.max(25, baseHum - Math.sin(i * 0.5) * 8.5),
            ).toFixed(1),
          ),
        });
      }
  ...
  const handleExportCSV = () => {
    ...
    const rows = chartDataPoints
      .map((p) => `${p.label},${p.temp},${p.hum}`)
      .join("\n");
    ...
    link.setAttribute(
      "download",
      `Sensor_Analytics_${timeRange}_${Date.now()}.csv`,
    );
