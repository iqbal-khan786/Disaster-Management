...
export function VillageDetailModal({
  ...
  const rain =
    node.rainMm !== undefined
      ? node.rainMm
      : node.rain !== undefined
        ? node.rain
        : 0;
  const soil =
    node.soilMoisture !== undefined
      ? node.soilMoisture
      : node.soil !== undefined
        ? node.soil
        : 0;
  const smoke =
    node.smokeLevel !== undefined
      ? node.smokeLevel
      : node.smoke !== undefined
        ? node.smoke
        : 0;
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div
        ...
        style={{
          ...
          padding: "24px",
          boxShadow:
            "0 25px 60px rgba(0,0,0,0.8), 0 0 25px rgba(56, 189, 248, 0.2)",
          display: "flex",
      ...
      >
        ...
        >
          <div>
            ...
            >
              GPS: {node.latitude?.toFixed(4)}°N, {node.longitude?.toFixed(4)}°E
              • Sector: {node.district || "Rayagada District"}
            </p>
          </div>
        ...
        >
          <div>
            ...
            >
              ...
              {node.disasterType || "ENVIRONMENTAL MONITORING"}
            </div>
            <div
              style={{ fontSize: "10px", color: "#cbd5e1", marginTop: "2px" }}
            >
              Relayed over LoRa Mesh via Hop {node.hopCount || 1} • Signal
              Strength: {node.rssi || -65} dBm
            </div>
          </div>
          ...
          >
            <div
              style={{
                ...
                alignItems: "center",
                gap: "6px",
                color: node.flameDetected ? "#ef4444" : "#10b981",
              }}
            >
              <Zap size={16} />
              <span style={{ fontSize: "11px", fontWeight: 700 }}>
                Flame Optical Sensor
              </span>
            </div>
            <div
              style={{
