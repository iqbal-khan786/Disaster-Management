...
const NAGAVALI_RIVER_CHANNEL = [
  [19.2600, 83.3250],
  [19.2480, 83.3320],
  [19.2450, 83.3380], // Kashipur Sector Runoff
  [19.2320, 83.3550],
  [19.2220, 83.3750],
  [19.2150, 83.3900], // Chandili & Kolnara Corridor
  [19.2000, 83.4020],
  [19.1850, 83.4120], // Kumbhikota River Meander
  [19.1740, 83.4150],
  [19.1670, 83.4170], // Rayagada Town & DEOC Base Hub
  [19.1520, 83.4220],
  [19.1380, 83.4280]
];
...
export function PreDisasterImpactMap({
  ...
  const primaryNode = nodes['NODE_01'] || nodes['V1'] || Object.values(nodes)[0] || {};
  ...
  const threatState = useMemo(() => {
    ...
    let timelineStages = [
      { time: 'Current', title: 'Normal Baseline', desc: 'All river levels & slopes operating within safe parameters', status: 'active', color: '#10b981' }
    ];
    ...
    if (sc === 'flood' || rainIntensity >= 70 || risk >= 70) {
      ...
      timelineStages = [
        { time: 'T-0h to T+2h', title: 'Inundation Ingress', desc: 'Upstream surge arrives at Kashipur Valley; river breaches 2.85m danger mark', status: 'active', color: '#ef4444' },
        { time: 'T+2h to T+14h', title: 'Peak Flood Crest', desc: 'Maximum inundation (14.8 km²), road blockages & low culverts submerged', status: 'upcoming', color: '#f97316' },
        { time: 'T+14h to T+36h', title: 'Gradual Recession', desc: 'Floodwaters drain into lower Nagavali channel; water level drops below danger line', status: 'upcoming', color: '#f59e0b' },
        { time: 'T+36h to T+64h', title: 'Recovery & Repatriation', desc: 'Debris clearing, sanitation, structural inspections & return to villages', status: 'upcoming', color: '#10b981' }
      ];
      ...
      timelineStages = [
        { time: 'T-0h to T+4h', title: 'Heavy Runoff Ingress', desc: 'Soil saturation reaches 92%; agricultural bank overflow', status: 'active', color: '#f59e0b' },
        { time: 'T+4h to T+18h', title: 'Continuous Waterlogging', desc: 'Low-lying roads waterlogged (0.6m - 1.2m depth)', status: 'upcoming', color: '#f97316' },
        { time: 'T+18h to T+36h', title: 'Drainage & Clearance', desc: 'Natural drainage through river network; all pathways cleared', status: 'upcoming', color: '#10b981' }
      ];
      ...
      timelineStages = [
        { time: 'T-0h to T+3h', title: 'Slope Shift & Mudflow', desc: 'Seismic vibration triggers slope failure; debris blocks valley access', status: 'active', color: '#ef4444' },
        { time: 'T+3h to T+24h', title: 'Secondary Rockfall Risk', desc: 'Geotechnical monitoring active; perimeter cordoned off', status: 'upcoming', color: '#f97316' },
        { time: 'T+24h to T+72h', title: 'Heavy Earthmover Clearance', desc: 'SDRF road clearance & hill slope reinforcement', status: 'upcoming', color: '#10b981' }
      ];
      ...
      timelineStages = [
        { time: 'T-0h to T+6h', title: 'Gale Inundation Front', desc: 'Squally winds 110km/h + 145mm torrential cloudburst', status: 'active', color: '#ef4444' },
        { time: 'T+6h to T+20h', title: 'Cyclone Eye Passage', desc: 'Storm surge inundation across low-lying rural wards', status: 'upcoming', color: '#f97316' },
        { time: 'T+20h to T+48h', title: 'Post-Storm Restoration', desc: 'Power grid repair, LoRa telemetry sync & road clearing', status: 'upcoming', color: '#10b981' }
      ];
      ...
      timelineStages = [
        { time: 'T-0h to T+4h', title: 'Thermal Spread Peak', desc: 'Smoke 380+ PPM, flame detected in dry canopy', status: 'active', color: '#ef4444' },
        { time: 'T+4h to T+14h', title: 'Containment Line Creation', desc: 'Forest department firebreak line established', status: 'upcoming', color: '#f97316' },
        { time: 'T+14h to T+24h', title: 'Air Quality Normalization', desc: 'Gas levels drop below 30 PPM; safe to return', status: 'upcoming', color: '#10b981' }
      ];
    }
  ...
  }, [activeScenario, primaryNode, rainIntensity, soilSat, isVibrating, isFlaming, smokeVal]);
    ...
    return [
      {
        ...
        impactDurationHours: isEmerg ? '48 - 72 Hours (3 Days)' : (isWarn ? '24 - 36 Hours' : '0 Hours (Nominal)'),
        waterloggingDepth: isEmerg ? '2.45 - 2.85 meters' : (isWarn ? '0.8 - 1.2 meters' : '0.0 m (Nominal)'),
        recessionEta: isEmerg ? 'Recession begins in ~36 Hours' : (isWarn ? 'Recession in 18 Hours' : 'Continuous Normal Flow'),
      ...
      },
      {
        ...
        impactDurationHours: isEmerg ? '24 - 36 Hours' : (isWarn ? '18 - 24 Hours' : '0 Hours (Nominal)'),
        waterloggingDepth: isEmerg ? '1.20 - 1.65 meters' : (isWarn ? '0.4 - 0.7 meters' : '0.0 m'),
        recessionEta: isEmerg ? 'Recession begins in ~24 Hours' : (isWarn ? 'Recession in 12 Hours' : 'Nominal'),
  ...
  const activePlace = disasterPlaces.find(p => p.id === selectedPlaceId) || disasterPlaces[0];
  ...
  const filteredPlaces = disasterPlaces.filter(p =>
  ...
  useEffect(() => {
    ...
    map.on('mousemove', (e) => {
      const latRatio = (e.latlng.lat - 19.1670) / (19.2450 - 19.1670);
      const estElev = Math.round(210 + (latRatio * 250) + Math.sin(e.latlng.lng * 10) * 50);
      setCursorCoords({
        lat: e.latlng.lat,
        lng: e.latlng.lng,
  ...
  const handleSelectPlaceCard = (place) => {
    setSelectedPlaceId(place.id);
    if (mapInstanceRef.current) {
      mapInstanceRef.current.flyTo([place.lat, place.lng], 14, { duration: 1.0 });
    }
  };
  ...
  return (
    <div style={{
      ...
      <div className="glass-panel" style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{
              background: threatState.severity === 'CRITICAL' ? 'rgba(239, 68, 68, 0.25)' : 'rgba(56, 189, 248, 0.25)',
              padding: '8px',
              borderRadius: '10px',
              color: threatState.severity === 'CRITICAL' ? '#ef4444' : '#38bdf8',
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                ...
                <span style={{
                  ...
                  padding: '2px 8px',
                  borderRadius: '4px',
                  background: threatState.severity === 'CRITICAL' ? 'rgba(239, 68, 68, 0.25)' : 'rgba(16, 185, 129, 0.25)',
                  color: threatState.severity === 'CRITICAL' ? '#fca5a5' : '#34d399',
                  border: `1px solid ${threatState.severity === 'CRITICAL' ? '#ef4444' : '#10b981'}`
                }}>
                  {threatState.severity === 'CRITICAL' ? '● CRITICAL INUNDATION SURGE ADVANCING' : '● ALL BASIN SECTORS NOMINAL'}
                </span>
              </div>
        ...
        <div style={{
          ...
          <div className="glass-card" style={{ padding: '12px 14px', borderLeft: '4px solid #ef4444' }}>
          ...
          <div className="glass-card" style={{ padding: '12px 14px', borderLeft: '4px solid #38bdf8' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#94a3b8', fontWeight: 600 }}>
              <span>Earliest Warning Lead-Time</span>
              <Clock size={15} color="#38bdf8" />
            </div>
            <div style={{ fontSize: '22px', fontWeight: 900, color: '#38bdf8', fontFamily: 'JetBrains Mono', margin: '4px 0' }}>
              {threatState.severity === 'CRITICAL' ? '42 Mins Advance' : 'Real-Time Telemetry'}
            </div>
            <div style={{ fontSize: '10px', color: '#cbd5e1' }}>
          ...
          <div className="glass-card" style={{ padding: '12px 14px', borderLeft: '4px solid #f59e0b', background: 'rgba(245, 158, 11, 0.08)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#fbbf24', fontWeight: 700 }}>
              <span>Disaster Impact Duration</span>
              <Hourglass size={15} color="#f59e0b" />
            </div>
            <div style={{ fontSize: '22px', fontWeight: 900, color: '#fbbf24', fontFamily: 'JetBrains Mono', margin: '4px 0' }}>
              {threatState.severity === 'CRITICAL' ? `${threatState.totalDurationHours} Hours (~3 Days)` : '0 Hours (Nominal)'}
            </div>
          ...
          <div className="glass-card" style={{ padding: '12px 14px', borderLeft: '4px solid #a855f7' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#94a3b8', fontWeight: 600 }}>
        ...
        {threatState.isDisasterImminent && (
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '6px' }}>
              <span style={{ fontSize: '11px', fontWeight: 800, color: '#fbbf24', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <Calendar size={13} /> Multi-Stage Disaster Impact & Recession Timeline Horizon:
              </span>
              <span style={{ fontSize: '10px', color: '#94a3b8', fontFamily: 'JetBrains Mono' }}>
              ...
              {threatState.timelineStages.map((stage, idx) => (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px' }}>
                    <span style={{ fontWeight: 800, color: stage.color }}>{stage.time}</span>
                    <span style={{ color: '#cbd5e1', fontWeight: 700 }}>{stage.title}</span>
                  </div>
      ...
      <div className="glass-panel" style={{
        ...
        <div style={{ position: 'relative', width: '100%', height: isFullscreen ? '700px' : '390px', overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  background: activeLayerKey === 'hybrid' ? '#1a73e8' : '#ffffff',
              ...
              <button
                ...
                style={{
                  background: activeLayerKey === 'roadmap' ? '#1a73e8' : '#ffffff',
              ...
              <button
                ...
                style={{
                  background: activeLayerKey === 'terrain' ? '#1a73e8' : '#ffffff',
      ...
      <div className="glass-panel" style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
          ...
          <span style={{ fontSize: '11px', color: '#94a3b8' }}>
            *Click any sector below to inspect lead-time, flood depth, and duration on map
          </span>
        </div>
              ...
              >
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '11px' }}>
                    ...
                    <span style={{ fontWeight: 900, color: place.pinColor, fontFamily: 'JetBrains Mono' }}>
                      {place.leadTimeText}
                    </span>
                  </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '11px' }}>
                    <span style={{ color: '#fbbf24', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <Hourglass size={12} color="#fbbf24" /> Affecting Duration:
                    </span>
                    <span style={{ fontWeight: 900, color: '#fbbf24', fontFamily: 'JetBrains Mono' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#cbd5e1' }}>
                  <span><b>Population:</b> {place.population.toLocaleString()} ({place.wardsCount} Wards)</span>
                  <span><b>Peak Depth:</b> <span style={{ color: '#38bdf8', fontWeight: 700 }}>{place.waterloggingDepth}</span></span>
                </div>

                {/* Safe Shelter Destination */}
                <div style={{ fontSize: '11px', color: '#34d399', display: 'flex', alignItems: 'center', gap: '5px' }}>
                  <ShieldCheck size={13} color="#34d399" />
                  <span><b>Assigned Safe Shelter:</b> {place.safeShelter}</span>
                </div>
              </div>
            );
          })}
      ...
      {activePlace && (
        <div className="glass-panel" style={{
          ...
          flexWrap: 'wrap',
          gap: '14px',
          background: 'linear-gradient(180deg, rgba(20, 36, 62, 0.98) 0%, rgba(12, 22, 38, 0.98) 100%)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <div style={{ fontSize: '13px', fontWeight: 900, color: '#ffffff' }}>
                Google Maps Evacuation Corridor: <span style={{ color: '#38bdf8' }}>{activePlace.name} ➔ {activePlace.safeShelter}</span>
              </div>
              <div style={{ fontSize: '11px', color: '#cbd5e1', marginTop: '2px' }}>
                Distance: <b>{activePlace.evacuationRouteDistance}</b> • Expected Displacement Horizon: <b>{activePlace.shelterStayDays}</b> • Status: <span style={{ color: '#34d399', fontWeight: 700 }}>HIGH-GROUND ASCENT CLEAR</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            {onOpenDispatch && (
              <button
                onClick={() => onOpenDispatch({ id: activePlace.nodeId || 'NODE_01', name: activePlace.name, latitude: activePlace.lat, longitude: activePlace.lng })}
                style={{
                  background: 'linear-gradient(135deg, #ef4444, #b91c1c)',
                  color: '#ffffff',
                  border: 'none',
            ...
            {onSelectNode && (
              <button
                onClick={() => onSelectNode(nodes[activePlace.nodeId] || nodes['NODE_01'] || { id: activePlace.nodeId, name: activePlace.name })}
                style={{
