...
export function useDisasterWebSocket() {
  ...
  const normalizeNodeData = useCallback((raw) => {
    ...
    return {
      ...
      rssi: rssi,
      snr: snr,
      accuracy: raw.accuracy !== undefined ? Number(raw.accuracy) : Number(Math.max(85.0, Math.min(95.0, 91.8 + (snr - 8.6) * 0.4)).toFixed(1)),
      accuracyPct: raw.accuracyPct !== undefined ? Number(raw.accuracyPct) : Number(Math.max(85.0, Math.min(95.0, 91.8 + (snr - 8.6) * 0.4)).toFixed(1)),
      dataAccuracy: raw.dataAccuracy !== undefined ? Number(raw.dataAccuracy) : Number(Math.max(85.0, Math.min(95.0, 91.8 + (snr - 8.6) * 0.4)).toFixed(1)),
      lastSeen: Date.now(),
      risk: riskTypes,
      status: "ONLINE",
