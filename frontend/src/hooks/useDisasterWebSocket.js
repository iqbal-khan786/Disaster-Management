...
function parseAnyIncomingData(rawInput) {
  ...
  if (text.includes("|")) {
    ...
    const cleanText =
      firstWordStart >= 0 ? text.substring(firstWordStart + 1) : text;
    ...
    if (parts.length >= 4) {
      ...
      for (let i = 1; i < parts.length; i++) {
        ...
        else if (p.startsWith("EVT:"))
          disasterType = p.replace("EVT:", "").trim();
        else if (p.startsWith("SCORE:"))
          riskScore = parseFloat(p.replace("SCORE:", "")) || 18.5;
        else if (p.startsWith("SEQ:"))
          seq = parseInt(p.replace("SEQ:", ""), 10) || 1000;
        else if (p.startsWith("HOP:"))
          hop = parseInt(p.replace("HOP:", ""), 10) || 1;
        ...
        } else if (p.startsWith("FLM:"))
          flm = p.includes("1") || p.includes("true") || p.includes("TRUE");
        else if (p.startsWith("VIB:"))
          vib = p.includes("1") || p.includes("true") || p.includes("TRUE");
...
export function useDisasterWebSocket() {
  ...
  const [logs, setLogs] = useState([
    {
      id: 1,
      time: new Date().toLocaleTimeString(),
      text: "[DisasterGuard CORE] Rayagada DEOC Command Center Initialized (6 Physical Sensors).",
      type: "sys",
    },
    {
      id: 2,
      time: new Date().toLocaleTimeString(),
      text: "[Gateway Bridge] Active on ws://127.0.0.1:8080.",
      type: "sys",
    },
  ]);
  ...
  const normalizeNodeData = useCallback(
    (raw) => {
      if (!raw) return null;
      const mappedId = "NODE_01";
      ...
      if (raw.soil_moisture !== undefined)
        soilMoisture = Number(raw.soil_moisture);
      else if (raw.soilMoisture !== undefined)
      ...
      else if (raw.smokeLevel !== undefined)
        smokeLevel = Number(raw.smokeLevel);
      ...
      if (raw.flame_detected !== undefined)
        flameDetected = Boolean(raw.flame_detected);
      else if (raw.flameDetected !== undefined)
        flameDetected = Boolean(raw.flameDetected);
      else if (raw.flame !== undefined)
        flameDetected =
          typeof raw.flame === "boolean" ? raw.flame : Number(raw.flame) > 0;
      else if (existing) flameDetected = existing.flameDetected;

      const flameVoltage =
        raw.flameVoltage !== undefined
          ? Number(raw.flameVoltage)
          : flameDetected
            ? 0.38
            : 3.26;
      ...
      // 5. Vibration (SW-420)
      const vibration =
        raw.vibration !== undefined
          ? Boolean(raw.vibration)
          : raw.vibration_detected !== undefined
            ? Boolean(raw.vibration_detected)
            : existing
              ? existing.vibration
              : false;
      const vibrationFreq =
        raw.vibrationFreq !== undefined
          ? Number(raw.vibrationFreq)
          : raw.vibrationHz !== undefined
            ? Number(raw.vibrationHz)
            : existing?.vibrationFreq || (vibration ? 38.0 : 0.0);
      const vibrationG =
        raw.vibrationG !== undefined
          ? Number(raw.vibrationG)
          : vibration
            ? 0.75
            : 0.02;
      ...
      else if (raw.humidityPct !== undefined)
        humidity = Number(raw.humidityPct);
      ...
      const rssi =
        raw.rssi !== undefined
          ? Number(raw.rssi)
          : raw.rssi_dbm !== undefined
            ? Number(raw.rssi_dbm)
            : existing
              ? existing.rssi
              : -67;
      const snr =
        raw.snr !== undefined ? Number(raw.snr) : existing?.snr || 8.6;
      const hopCount = raw.hopCount !== undefined ? Number(raw.hopCount) : 1;
      const packetSequence =
        raw.packetSequence !== undefined
          ? Number(raw.packetSequence)
          : existing?.packetSequence
            ? existing.packetSequence + 1
            : 1024;

      // Weighted risk computed from strictly the 6 physical sensors with calibrated ambient baselines
      let calculatedRiskScore =
        raw.riskScore !== undefined ? Number(raw.riskScore) : null;
      if (calculatedRiskScore === null) {
        const rainScore =
          16.0 + (rainMm > 0 ? (Math.min(75, rainMm) / 75.0) * 84.0 : 0.0);
      ...
      return {
        ...
        village: "Kashipur Valley",
        district: "Rayagada, Odisha",
        latitude: raw.latitude !== undefined ? raw.latitude : 19.195,
        longitude: raw.longitude !== undefined ? raw.longitude : 83.395,
        elevation: 310,
        ...
        accuracy:
          raw.accuracy !== undefined
            ...
            : Number(
                Math.max(
                  85.0,
                  Math.min(95.0, 91.8 + (snr - 8.6) * 0.4),
                ).toFixed(1),
              ),
        accuracyPct:
          raw.accuracyPct !== undefined
            ? Number(raw.accuracyPct)
            : Number(
                Math.max(
                  85.0,
                  Math.min(95.0, 91.8 + (snr - 8.6) * 0.4),
        ...
        dataAccuracy:
          raw.dataAccuracy !== undefined
            ? Number(raw.dataAccuracy)
            : Number(
                Math.max(
                  85.0,
                  Math.min(95.0, 91.8 + (snr - 8.6) * 0.4),
        ...
        rawPacket:
          raw.rawPacket ||
          `V1|SEQ:${packetSequence}|LVL:${calculatedRiskLevel}|EVT:${disasterType}|RAIN:${rainMm}mm|SOIL:${soilMoisture}%|SMK:${smokeLevel}PPM|FLM:${flameDetected ? "1" : "0"}|VIB:${vibration ? "1" : "0"}|TEMP:${temp}C|HUM:${humidity}%|RSSI:${rssi}dBm`,
      };
    },
    [nodes],
  );
    ...
    (data) => {
      ...
      items.forEach((rawItem) => {
        ...
        if (
          ...
          setAlerts((prev) => [
            newAlert,
            ...prev.filter((a) => a.id !== newAlert.id).slice(0, 19),
          ]);
        }
        ...
        addLog(
          ...
          node.riskLevel === "CRITICAL" || node.riskLevel === "EMERGENCY"
            ? "danger"
    ...
    (url) => {
      ...
      try {
        ...
        ws.onopen = () => {
          ...
          addLog(
            `[WS Client] Connected to DisasterGuard Backend (${targetUrl})`,
            "ack",
          );
        ...
        ws.onclose = () => {
          ...
          addLog(
            `[WS Client] Disconnected from ${targetUrl}. Retrying in 3.5s...`,
            "warn",
          );
          if (!reconnectTimerRef.current) {
  ...
  const connectUsbSerial = useCallback(async () => {
    ...
    try {
      ...
      const portTitle = portInfo.usbVendorId
        ? `USB COM (VID:${portInfo.usbVendorId.toString(16)})`
        : "ESP32 Serial";
  ...
  const dispatchRescue = useCallback(
    (nodeId, coords, details = {}) => {
      playEmergencySiren(1.5, 0.2);
      sendCommand("DISPATCH", { node: nodeId, coords, ...details });
      addLog(
        `[DISPATCH] Rescue Unit Alpha mobilized for ${nodeId} (${coords})`,
        "danger",
      );
    },
    [sendCommand, addLog],
  ...
  const setSimulationScenario = useCallback(
    (scenario) => {
      setActiveScenario(scenario);
      playTacticalBeep(1600, 0.12);
      addLog(
        `[SCENARIO] Switched to Scenario: ${scenario.toUpperCase()}`,
        "sys",
      );
  ...
  const toggleDemoMode = useCallback(() => {
    setIsDemoMode((prev) => {
      const next = !prev;
      addLog(
        `[MODE] Switched to ${next ? "DEMO SIMULATION MODE" : "LIVE HARDWARE MODE"}`,
        next ? "warn" : "ack",
      );
      if (!next) {
  ...
  useEffect(() => {
    const offlineTimer = setInterval(() => {
      ...
      setNodes((prev) => {
        ...
        const newRain =
          current.rainMm > 0 ? Number(current.rainMm.toFixed(1)) : 0.0;
        const newSoil = Number(
          Math.max(
            5.0,
            Math.min(99.5, (current.soilMoisture || 34.2) + dSoil),
          ).toFixed(1),
        );
        const newSmoke = Number(
          Math.max(
            10.0,
            Math.min(800.0, (current.smokeLevel || 18.4) + dSmoke),
          ).toFixed(1),
        );
        const newTemp = Number(
          Math.max(
            15.0,
            Math.min(50.0, (current.temp || 26.4) + dTemp),
          ).toFixed(1),
        );
        const newHum = Number(
          Math.max(
            15.0,
            Math.min(99.0, (current.humidity || 64.5) + dHum),
          ).toFixed(1),
        );
